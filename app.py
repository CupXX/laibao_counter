...
def main():
    ...
    with col1:
        ...
        4. **设置码数**: 设置每个接龙的码数，默认为1，可自行修改，设置好后点击开始处理按钮，即可自动计算积分。<br>
        5. **查看排行榜**: 在主页面下方可查看积分排行榜和已处理文件列表。<br>
        6. **下载积分表格**: 点击积分排行榜右上方的下载按钮可以下载当前的排行榜数据。<br>
        7. **备份数据**: 在左侧侧边栏，点击【下载我的数据】，即可备份所有处理历史记录，保存为json文件。<br>
        8. **上传数据**: 再次使用时，把保存的json文件拖到【上传数据】区域，即可上传之前备份的数据，继续编辑。
        </div>
        """, unsafe_allow_html=True)
